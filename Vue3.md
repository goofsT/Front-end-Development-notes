...
```js
父绑定：<Child  @myEvent="handler"></Child>
...
<template>
  <div>
    <h1>我是子组件2</h1>
    <button @click="sendData">点击我触发xxx自定义事件</button>
  </div>
</template>
...
**注意，如果原生的事件(click,change..)使用defineEmites定义后，就会变为自定义事件**

### 1.3全局事件总线

在Vue3想实现全局事件的总线功能就有点不现实啦，如果想在Vue3中使用全局事件总线功能可以使用插件mitt实现。

**mitt:官网地址:https://www.npmjs.com/package/mitt**

```js
src新建bus文件夹，创建index.ts
...
Bro2组件：
<button @click=handleChange>给子2发数据</button>
import bus from '../../bus'
handleChange=()=>{
    bus.emit('sendData',{name:'张三',age:18})
}
...
假如此时，子组件接收到父组件props传递过来的数据，当我子组件的数据发生改变时，父组件的值也进行相应的更新
父：
<Child :modelValue="money" @update:modelValue="updateMoney"/>
import {ref} form 'vue'
let money=ref(1000)
updateMoney=(num)=>{
    money.value=num
...
子：
<h3>{{money}}<h3/>
<button @click="handle">点击同步数据</button>

let props=defineProps(['modelValue'])
let $emit=defineEmits(['update:modelValue'])
const handle=()=>{
    $emit('update:money',props.modelValue--)
}


父：<Child :modelValue="money" @update:modelValue="updateMoney"/>
上方代码可以直接简写为：
父：<Child v-model="money"/>
...
> 子组件内部可以通过useAttrs方法获取组件属性与事件，如果用defineProps接收，则无法获取到对应的属性
...
let useUserStore=defineStroe('User',{
    //存储数据
    state:()=>{
        retrun{
            token:localStorage.getItem('userToekn')
        }
    },
    //逻辑
    action:{
        //登录方法
        //async函数执行返回一个Promise,结果由函数结果决定，undefine为成功的Promise
...
组件中使用：
let useUserStore from '@/stroe/module/user'
...
const handleLogin=async()=>{
    try{
        ...
        ELNotification({
            type:'error',
            message:(err as Error).message //ts类型断言
        })
	}
...
> 在components文件夹目录下创建一个index.ts文件,注册所有全局组件

```js
引入全局组件
import SvgIcon from './SvgIcon/index.vue';//全局组件
import Test from './test.vue' //全局组件
const obj={SvgIcon,Test}
export default {
    install(app) {
        Object.keys(obj).forEach(key => {
            app.component(key,obj[k]);//注册为全局组件
        })
    }
}
```

> 入口文件引入src/index.ts文件,通过app.use方法安装自定义插件

```js
import gloablComponent from './components/index';
app.use(gloablComponent);//安装插件，触发install方法
```
...
在style/variable.scss创建一个variable.scss文件存放全局样式变量

```js
在variable.scss中定义全局样式变量:
$base-color:red;
...
1:使用请求拦截器，可以在请求拦截器中处理一些业务(开始进度条、请求头携带公共参数,如token)
...
let request = axios.create({
    baseURL: import.meta.env.VITE_APP_BASE_API,//开发环境变量
    timeout: 5000
})
//请求拦截器
request.interceptors.request.use(config => {
    config配置对象中有headers属性请求头	
    return config;
});
//响应拦截器
